...
Physics::Point Physics::ClosestPtPointTriangle( Point p, Point a, Point b, Point c, bool &bIsInsideFaceRegion)
{
	...
	CVector3f ab = b - a;		// Vector ab = b - a;
	CVector3f ac = c - a;		// Vector ac = c - a;
	CVector3f ap = p - a;		// Vector ap = p - a;
	float d1 = CVector3f::DotProduct( ab, ap );		// float d1 = Dot(ab, ap);
	float d2 = CVector3f::DotProduct( ac, ap );		// float d2 = Dot(ac, ap);
	...
	CVector3f bp = p - b;							// Vector bp = p - b;
	float d3 = CVector3f::DotProduct( ab, bp );		// float d3 = Dot(ab, bp);
	float d4 = CVector3f::DotProduct( ac, bp );		// float d4 = Dot(ac, bp);
	...
	CVector3f cp = p - c;							// Vector cp = p - c;
	float d5 = CVector3f::DotProduct( ab, cp );		// float d5 = Dot(ab, cp);
	float d6 = CVector3f::DotProduct( ac, cp );		// float d6 = Dot(ac, cp);
	if ( (d6 >= 0.0f) && (d5 <= d6) )
	{
