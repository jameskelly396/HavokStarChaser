...
in vec4 ex_DebugObjectColour;	// (no lighthing colour)
...
void main(void)
{
	...
	if ( bIsSkyMap )
	{
		...
		//out_Color = vec4( 1.0f, 0.25f, 0.1f, 1.0f );
		out_Color = texture( skyMapTexture, ex_Normal.xyz );
	}
	...
	else if ( bUseDebugColour )
	{
		...
		out_Color *= vec4( 2.0f, 2.0f, 2.0f, 1.0f );
	}
	else
	...
	//calcLightContrib(out_Color, surfaceColor, ex_Position, ex_Normal);
}
